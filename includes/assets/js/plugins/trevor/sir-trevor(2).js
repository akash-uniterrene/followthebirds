...
(function($, _) {
    ...
    */
    ...
    (function(a, b, c) {
        ...
        p.defaults = {}, m(p.prototype, { spin: function(a) { this.stop();
                var b = this,
                    c = b.opts,
                ...
                var a = this.el;
                return a && (clearTimeout(this.timeout), a.parentNode && a.parentNode.removeChild(a), this.el = c), this }, lines: function(a, b) {
                function e(a, d) {
                ...
                return a }, opacity: function(a, b, c) { b < a.childNodes.length && (a.childNodes[b].style.opacity = c) } }), ! function() {
            function a(a, b) {
                return g("<" + a + ' xmlns="urn:schemas-microsoft.com:vml" class="spin-vml">', b) }
            var b = l(g("group"), { behavior: "url(#default#VML)" });!k(b, "transform") && b.adj ? (i.addRule(".spin-vml", "behavior:url(#default#VML)"), p.prototype.lines = function(b, c) {
                function f() {
                ...
                return h(b, i) }, p.prototype.opacity = function(a, b, c, d) {
                var e = a.firstChild;
    ...
    */
    ...
    SirTrevor.toMarkdown = function(content, type) {
        ...
        var inlineTags = ["em", "i", "strong", "b", "u"];
        ...
        function replaceUnderline(match, p1, p2) {
            if (_.isUndefined(p2)) { p2 = ''; }
            return "~" + p1.replace(/<(.)?br(.)?>/g, '') + "~" + p2;
        }
        ...
        markdown = markdown.replace(/<(\w+)(?:\s+\w+="[^"]+(?:"\$[^"]+"[^"]+)?")*>\s*<\/\1>/gim, '') //Empty elements
            ...
            .replace(/<u>(?:\s*)(.*?)(\s*)?<\/u>/gim, replaceUnderline)
            ...
            */
            ...
            if (!_.isUndefined(types) &&
                _.some(types, function(type) {
                    return _.include(this.valid_drop_file_types, type); }, this)) {
    ...
    SirTrevor.BlockDecoration = (function() {

        var BlockDecoration = function() {
            this._ensureElement();
            this._bindFunctions();
        };

        _.extend(BlockDecoration.prototype, FunctionBind, Renderable, {

            tagName: 'a',
            className: 'st-block-ui-btn st-block-ui-btn--decoration st-icon',
    ...
    SirTrevor.Block = (function() {
        ...
        var t_test = [
            '<div class="option-panel style_options">',
            ...
            '<a href="javascript:void(0)"  data-value="right" class="btn btn-primary btn_text_align">Right</a>',
            '</div></p></li>',
            '</ul>',
        ...
        _.extend(Block.prototype, SirTrevor.SimpleBlock.fn, SirTrevor.BlockValidations, {
            ...
            bound: ["_handleContentPaste", "_onFocus", "_onBlur", "onDrop", "onDeleteClick", "onstyle",
            ...
            initialize: function() {
                this.$b = $(document);
                console.log(this.$b);
                //this.$b.on('click', '.save_the_styles', this.saveInitRanit);
            },
            saveInitRanit: function (argument) {
              alert('dsf');
            },
            ...
            */
            ...
            check_again:function(e){
              
              $(document).on('click','.save_the_styles',function(){
                  alert();
              }) ;
            },

            onstyle: function(e) {
                if (this.$inner.find('.style_options').length > 0) {
                    this.$inner.find('.style_options').addClass('showNow');
                } else {
                    var ll = this.$inner;
                    this.$inner.append(_.template(t_test));
                    setTimeout(function(argument) {
                        ll.find('.style_options').addClass('showNow');
                    }, 500);
                }
                this.check_again()
            
                /*      
         //if(this.$el.find('#pop_'+this.blockID).length > 0){
                   // this.$el.find('#pop_'+this.blockID).addClass('show');
                // }else{
                   // this.$inner.append(_.template(t_test)); 
                //   // this.$editor.append(_.template(t_test));
                //   }

                this.$el.find('.heading-blocks > div').attr('id','pop_'+this.blockID);
                this.$el.find('.heading-blocks #pop_'+this.blockID).removeClass('hide');
                this.$el.find('.heading-blocks > div').addClass('show');
               // this.$el.find('.heading-block > div .close').on('click', hideThePopup(this)); */
            },
            ...
            _initUIComponents: function() {
                ...
                this._withUIComponent(
                    new SirTrevor.BlockDecoration(this.$el), '.st-block-ui-btn--decoration', this.onstyle
                );
    ...
    */
    ...
    SirTrevor.Blocks.Columns = (function() {
        return SirTrevor.Block.extend({
            ...
            editorHTML: function() {
                return _.template(
                    '<div class="columns-row" id="<%= blockID %>-columns-row" style="overflow: auto"/>', { blockID: this.blockID })
            },
            ...
            applyColumns: function(preset, initial) {
                ...
                var total_width = _.reduce(columns_config, function(total, width) {
    ...
    Heading Block
  */
  var t_test = '<div class="option-panel style_options">'+
            '<div class="option-popup">'+
            '<a href="javascript:void(0)" onclick="$(this).parent().parent().removeClass(\'showNow\')" ><i class="fa fa-times"></i></a>'+
            '<ul>'+
            '<li> <p> Font Size : <input  type="text" name="style_font" id="style_font" size="3" /> </p> </li>'+
    ...
    SirTrevor.Blocks.Heading = SirTrevor.Block.extend({
        ...
        editorHTML: '<div class="heading-block"><select name="heading" class="form-control"><option value="h1">H1</option><option value="h2">H2</option><option value="h3">H3</option><option value="h4">H4</option><option value="h5">H5</option><option value="h6">H6</option></select></div><div class="st-required st-text-block st-text-block--heading" contenteditable="true"></div>'+t_test,
    ...
    */
    ...
    SirTrevor.Blocks.Image = SirTrevor.Block.extend({
        ...
        loadData: function(data) {
            ...
            this.$editor.append($('<input type="radio" name="style" value="border-radius:50%">'));
        },
    ...
    SirTrevor.Blocks.ImageExtended = SirTrevor.Blocks.Image.extend({
        ...
        loadData: function(data) {
            // Create our image tag
            this.$editor.html($('<img>', { src: data.file.url, style: data.style })).show();
            ...
            this.$editor.append($('<input>', { type: 'hidden', name: 'style', value: data.style }));
        },
        onBlockRender: function() {
            /* Setup the upload button */
            this.$inputs.find('button').bind('click', function(ev) { ev.preventDefault(); });
    ...
    */
    ...
    SirTrevor.Blocks.Text = SirTrevor.Block.extend({
        ...
        editorHTML: '<div class="st-required st-text-block" contenteditable="true"></div>'+t_test,
    ...
    */
        ...
        return SirTrevor.Block.extend({
            ...
            loadData: function(data) {
                if (!this.providers.hasOwnProperty(data.source)) {
                    this.$editor.html($('<video>', { src: data.file.url,controls:'' })).show();
                }else{
    ...
    (function() {
        ...
        var Center = SirTrevor.Formatter.extend({
            title: "center",
            cmd: "justifyCenter",
            keyCode: 68,
            text: "C"
        ...
        */
        ...
        SirTrevor.Formatters.Center = new Center();
...
$(document).on('ready',function(){
   ...
   jQuery(document).on('keyup','#style_font',function(){ 
       
       var fontSize = 'Font-size:'+jQuery(this).val()+'px;';
       
 jQuery(this).parent().parent().parent().parent().parent().find('#test_r').attr('data-value',fontSize);
   });
   
  jQuery(document).on('click','.save_the_styles', function(){ jQuery(this).text() } );
