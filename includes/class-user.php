...
class User {
    ...
    * @return array
    */
    public function get_profile_templates($user_id, $offset = 0) {
        global $db, $system;
        $templates = array();
        $get_templates = $db->query("SELECT * FROM `profile_template` WHERE `user_id` = $user_id");
        if($get_templates->num_rows > 0) {
                while($row = $get_templates->fetch_assoc()) {
                    $templates[] = $row;
                }
        }
        return $templates;
    ...
    * @return array
    */
    public function get_page_templates($page_id, $offset = 0) {
        global $db, $system;
        $templates = array();
        $get_templates = $db->query("SELECT * FROM `page_template` WHERE `page_id` = $page_id");
        if($get_templates->num_rows > 0) {
                while($row = $get_templates->fetch_assoc()) {
                    $templates[] = $row;
                }
    ...
    * @return array
    */
    public function view_page_template($template_id){
        global $db, $system;
        $templates = $db->query("SELECT * FROM `page_template` WHERE `template_id` = $template_id");
        if($templates->num_rows > 0) {
            $row = $templates->fetch_assoc();
        }
        return $row;
    } 
    ...
    * @return array
    */
    public function view_profile_template($template_id){
        global $db, $system;
        $templates = $db->query("SELECT * FROM `profile_template` WHERE `template_id` = $template_id");
        if($templates->num_rows > 0) {
            $row = $templates->fetch_assoc();
        }
        return $row;
    ...
    * @return array
    */
    public function update_profile_template($set=array(),$where=array()){
        global $db;
        
        if($set['active'] == '1'){
			$db->query("UPDATE `profile_template` SET `active` = '0' WHERE `user_id` = '{$set['user_id']}'");
		}
		$template_content = addslashes($set['template_content']);
		$pageTitle = addslashes($set['pageTitle']);
		$template_content_html = addslashes($set['template_content_html']);
		
        $sql = "UPDATE profile_template SET user_id = '{$set['user_id']}', template_content = '{$template_content}', pageTitle = '{$pageTitle}',active = '{$set['active']}', template_content_html = \"{$template_content_html}\" WHERE template_id = '{$where['template_id']}'";
    ...
    * @return array
    */
    public function get_active_template($page_id){
        global $db, $system;
        $template = $db->query("SELECT *  FROM `page_template` WHERE `page_id` = $page_id AND `active` = '1'");
        if($template->num_rows > 0) {
            $row = $template->fetch_assoc();
        }
        return $row;
    }
	
	 /**
    * get_active_profile_template
    * 
    * @param integer $page_id
    * @return array
    */
    public function get_active_profile_template($user_id){
        global $db, $system;
        $template = $db->query("SELECT * FROM `profile_template` WHERE `user_id` = $user_id AND `active` = '1'");
        if($template->num_rows > 0) {
            $row = $template->fetch_assoc();
        }
        return $row;
    ...
    */
    public function create_template($params){
        global $db;
		if($params['active'] == '1'){
			$db->query("UPDATE `profile_template` SET `active` = '0' WHERE `user_id` = '{$params['user_id']}'");
		}
		$template_content = addslashes($params['template_content']);
		$pageTitle = addslashes($params['pageTitle']);
		$template_content_html = addslashes($params['template_content_html']);
        $sql = "INSERT INTO `profile_template` ( `user_id`,`template_content`,`pageTitle`,`template_content_html`,`active`) VALUES
 ( '{$params['user_id']}','{$template_content}','{$pageTitle}',\"{$template_content_html}\", '{$params['active']}')";
        $db->query($sql) or _error(SQL_ERROR_THROWEN);
    }
    ...
    */
    public function save_template($params){
        global $db;
		if($params['active'] == '1'){
			$db->query("UPDATE `page_template` SET `active` = '0' WHERE `page_id` = '{$params['page_id']}'");
		}
		$template_content = addslashes($params['template_content']);
		$pageTitle = addslashes($params['pageTitle']);
		$template_content_html = addslashes($params['template_content_html']);
        $sql = "INSERT INTO `page_template` ( `page_id`,`template_content`,`pageTitle`,`template_content_html`,`active`) VALUES
 ( '{$params['page_id']}','{$template_content}','{$pageTitle}',\"{$template_content_html}\", '{$params['active']}')";
        $db->query($sql) or _error(SQL_ERROR_THROWEN);
    }

	public function delete_template($id){
		global $db;
		$sql = "DELETE FROM `followdbirds`.`page_template` WHERE `page_template`.`template_id` = $id";
		$db->query($sql) or _error(SQL_ERROR_THROWEN);
	}
	public function delete_profile_template($id){
		global $db;
		$sql = "DELETE FROM `followdbirds`.`profile_template` WHERE `profile_template`.`template_id` = $id";
		$db->query($sql) or _error(SQL_ERROR_THROWEN);
	}
    /**
    ...
    * @return array
    */
    public function update_template($set=array(),$where=array()){
        global $db;
        
        if($set['active'] == '1'){
			$db->query("UPDATE `page_template` SET `active` = '0' WHERE `page_id` = '{$set['page_id']}'");
		}
		$template_content = addslashes($set['template_content']);
		$pageTitle = addslashes($set['pageTitle']);
		$template_content_html = addslashes($set['template_content_html']);
		
        $sql = "UPDATE page_template SET page_id = '{$set['page_id']}', template_content = '{$template_content}', pageTitle = '{$pageTitle}',active = '{$set['active']}', template_content_html = \"{$template_content_html}\" WHERE template_id = '{$where['template_id']}'";    
        $db->query($sql) or _error(SQL_ERROR_THROWEN);
    }
    /**
     * edit_page
