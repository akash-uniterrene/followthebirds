...
try {
	...
	if(!$user->_logged_in) {
		...
		if($system['system_random_profiles']) {
			...
			$get_random_profiles = $db->query("SELECT user_name, user_firstname, user_lastname, user_gender, user_picture FROM users ORDER BY RAND() LIMIT 10") or _error(SQL_ERROR_THROWEN);
