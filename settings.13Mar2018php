...
try {
	...
	switch ($_GET['view']) {
		...
		case 'templates';
			// page header
			page_header(__("Settings")." &rsaquo; ".__("Template Settings"));
			$templates = $user->get_profile_templates(secure($user->_data['user_id'], 'int'));
			$smarty->assign('templates', $templates);
			break;
		case 'create-template':
			page_header(__("Settings")." &rsaquo; ".__("Template Settings"));
			break;
		case 'edit_template-'.$split['1']:
			page_header(__("Settings")." &rsaquo; ".__("Template Settings"));
			$template = $user->view_profile_template($split['1']);
			$smarty->assign('template', $template);
			$smarty->assign('case', 'edit-template');
			break;
		case 'delete_template-'.$split['1']:
			page_header(__("Settings")." &rsaquo; ".__("Template Settings"));
			$template = $user->view_profile_template($split['1']);
			$smarty->assign('template', $template);
			$smarty->assign('case', 'delete-template');
			break;
		case 'attachments':					
				$attachment = $_POST['attachment'];
				$uploadedFile = $_FILES['attachment']['tmp_name']['file'];

				$uploadedDir = __DIR__.'/'.$system['uploads_directory'].'/page';
				$path = $system['uploads_directory'].'/page';
				$url = $system['system_url'].'/'.$path;
				// create an image name
				$fileName = strtotime(date('d-m-y h:i:s')).'_'.$attachment['name'];

				// upload the image
				move_uploaded_file($uploadedFile, $uploadedDir.'/'.$fileName);
				return_json(array('file' => array(
					'url' => $url . '/' . $fileName,
					'filename' => $fileName
					)));
			break;	
		case 'uploadVault':			
				...
				$uploadedDir = __DIR__.'/'.$system['uploads_directory'].'/user/id/vault';
		...
		case 'vault':
			// page header
			$uploadedDir = __DIR__.'/'.$system['uploads_directory'].'/user/*';
			$dirs = array_filter(glob($uploadedDir), 'is_dir');
			print_r( $dirs);
			page_header(__("Settings")." &rsaquo; ".__("Manage Vault"));
			break;
