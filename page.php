...
try {
	...
	switch ($_GET['view']) {
		case '':
			...
			/* active template */
			$spage['active_template'] = $user->get_active_template($spage['page_id']);		
			$json = json_decode($spage['active_template']['template_content'], true);
			$spage['jsonArray'] = $json;
		...
		case 'page-template':
			...
			switch ($sub_view) {
				case '':
					$spage['page_templates'] = $user->get_page_templates($spage['page_id']);
					$smarty->assign('case', 'list');
				break;	
				case 'create':	
					$smarty->assign('case', 'create');
				break;
				case 'edit':	
					$spage['view_template'] = $user->view_page_template($split['1']);
					$smarty->assign('case', 'edit');
				break;	
				case 'delete':	
					$spage['view_template'] = $user->view_page_template($split['1']);
					$smarty->assign('case', 'delete');
				break;	
				case 'attachments':
					...
					$fileName = strtotime(date('d-m-y')).'_'.$attachment['name'];
